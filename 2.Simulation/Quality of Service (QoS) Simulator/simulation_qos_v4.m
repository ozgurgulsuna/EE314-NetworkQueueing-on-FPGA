...
T_in_mean = 2.7;         % Input Data Mean Period 3 Seconds
T_in_jitter = 0 ;      % Input Data Jitter ±3 Seconds 

% weights
Wl=[0 0 0 0];          % Latency Weight Constant  
Wr=[1 1 1 1];          % Reliability Weight Constant
...
for i=1:length(T_time_array)
    ...
    switch T_input_array(i)
        case 1
            if buffer_fullness(1)==6
                ...
                all_data(find(all_data(:,1)==max(all_data(:,1)),1,"first"),1)=0; % remove the oldest data
            else
        ...
        case 2
            if buffer_fullness(2)==6
                ...
                all_data(find(all_data(:,2)==max(all_data(:,2)),1,"first"),2)=0; % remove the oldest data
        ...
        case 3 
            if buffer_fullness(3)==6
                ...
                all_data(find(all_data(:,3)==max(all_data(:,3)),1,"first"),3)=0; % remove the oldest data
                % do nothing
            else
        ...
        case 4 
            if buffer_fullness(4)==6
                ...
                all_data(find(all_data(:,4)==max(all_data(:,4)),1,"first"),4)=0; % remove the oldest data
                % do nothing
            else
    ...
    X=Wl.*t+Wr.*f; % Evaluate buffer priority
